package hbx.media
{
	...
	import hbx.core.IPlayer;
	import hbx.core.IStream;



	public class CBumVideoStream extends EventDispatcher implements IStream, IPlayer, IDisposable
	{
		//- 스트림에러
		...
		protected function ppOnMetaData(tInfoObj:Object):void
		{
			...
			{
				_MetaData = tInfoObj;
				var te:CEventCore = new CEventCore(ET_PLAY_READY);
		...
		protected function ppNetStreamNetStatusHandler(tEvt:NetStatusEvent):void
		{
			...
			switch (tCode)
			{
				case 'NetStream.Play.Stop':
					...
					{
						this.pause();
						this.dispatchEvent(new CEventCore(ET_PLAY_STOP));
					}
					break;

				case 'NetStream.Play.StreamNotFound':
					this.dispatchEvent(new CEventCore(ET_STREAM_ERROR));
					break;
			}
		}
		...
		protected function ppSeekCheck(tEvt:Event):void
		{
			...
			if (_MetaData != null)
			{
				this.dispatchEvent(new CEventCore(ET_POSITION_UPDATE));
			}
		}
