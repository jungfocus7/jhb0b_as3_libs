package hbx.media
{
	...
	public class CBumSound extends EventDispatcher implements IPlayer, IDisposable
	{
		...
		private function p_snd_loadComplete(evt:Event):void
		{
			if (this.autoPlay)
				this.play();
			this.dispatchEvent(new CEventCore(ET_LOAD_COMPLETE));
		}

		private function p_sc_playEnd(evt:Event):void
		{
			this.stop();
			this.dispatchEvent(new CEventCore(ET_PLAY_END));
		}
